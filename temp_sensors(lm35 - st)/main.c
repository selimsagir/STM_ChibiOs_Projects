...
static const ADCConversionGroup adcgrpcfg1 = {
  ...
  0,                        /* CR1 */
  ADC_CR2_SWSTART,          /* CR2 */
  ADC_SMPR1_SMP_AN11(ADC_SAMPLE_480) | ADC_SMPR1_SMP_SENSOR(ADC_SAMPLE_480),
  0,                        /* SMPR2 */
  ADC_SQR1_NUM_CH(ADC_GRP1_NUM_CHANNELS),
  0,                        /* SQR2 */
  ADC_SQR3_SQ2_N(ADC_CHANNEL_SENSOR) | ADC_SQR3_SQ1_N(ADC_CHANNEL_IN11) 
};
...
static THD_FUNCTION(Thread2, arg) {
  ...
  float V25; 
  float Avg_Slope;    //this values are in datasheet electricity characteristics section
  V25 = 0.76;
  Avg_Slope = 2.5e-3;
	  ...
	  while(true)
	   {
	    ...
	    dat.frame.temp1 = ((sample[0]*3.0/4096)/0.01) ;  // for lm35 temperature sensor	
	  	dat.frame.temp2 = ((sample[1]*3.0/4096-V25)/Avg_Slope) + 25;  //for stmicrocontroller temperature sensor
	  	chThdSleepMilliseconds(500);
	  	dat.frame.checksum=0;	
	  		for(unsigned ii=3; ii<11 ; ii++){	
			      dat.frame.checksum+=dat.data[ii];
	  		}
		  	for(unsigned i=0 ; i<sizeof(dat.data) ; i++){
		        chSequentialStreamPut((BaseSequentialStream*) &SDU1,dat.data[i]);   		
		  	}
